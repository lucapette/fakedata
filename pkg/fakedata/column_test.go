...
func TestNewColumns(t *testing.T) {
	tests := []struct {
		...
		{name: "one column", input: []string{"email"}, expected: fakedata.Columns{{Name: "email"}}},
		{name: "two columns", input: []string{"email", "domain"}, expected: fakedata.Columns{{Name: "email"}, {Name: "domain"}}},
	}
